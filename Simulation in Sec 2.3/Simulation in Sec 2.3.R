...
par.comb <- expand.grid(I = c(100, 500),
                        n_J = c(3, 5, 10))
par.comb <- par.comb[order(par.comb$I, par.comb$n_J), ]
  ...
  for(kkk in 1:nrow(par.comb)){
    ...
    results_kkk <- foreach::foreach(iii = 1:B,
                                    ...
                                    .combine = 'c') %dopar%
      {
        ...
        ## Reader effects
        r <- bridgedist::rbridge(n = n_J, phi = lambda) # random effects for readers
        ...
        ## Case effects
        v <- bridgedist::rbridge(n = n_I, phi = phi) / lambda # random effects for cases
        ...
        ## binary outcome
        pp <- plogis(beta_0 + beta_2k * z_ij2 + beta_3k * z_ij3 + r[j_vals] + v[i_vals])
        ...
        ########################################################################
        rm(n_I, n_J, r, beta0_control, beta0_coef, v, 
        ...
        fit_glmmTMB2 <- tryCatch({
          R.utils::withTimeout({
            withCallingHandlers({
              model <- glmmTMB::glmmTMB(formula = Y ~ Modality + (1 | ID) + (1 | Reader),
                                        family = binomial(link="logit"), 
                                        data = dat, REML = FALSE,
                                        control = glmmTMBControl(optCtrl = list(iter.max = 1e3, 
                                                                                eval.max = 1e3)))
        ...
        fit_glmer1_w_which <- ifelse(class(fit_glmer1)[1] == "logical", TRUE, 
                                     isSingular(fit_glmer1))
        ...
        fit_glmer2 <- tryCatch({
          R.utils::withTimeout({
            withCallingHandlers({
              model <- lme4::glmer(formula = Y ~ Modality + (1 | ID) + (1 | Reader),
                                   family = binomial(link = "logit"), data = dat)
              model
            }, warning = function(w) {
        ...
        fit_glmer2_w_which <- ifelse(class(fit_glmer2)[1] == "logical", TRUE, 
                                     isSingular(fit_glmer2))
...
################################################################################
# setwd("~/")
save.image(file = "Simulation in Sec 2.3.RData")
