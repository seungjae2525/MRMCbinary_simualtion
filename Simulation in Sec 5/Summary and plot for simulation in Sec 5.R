...
rm(list = ls())
load(file = "Simulation in Sec 5.RData")

## Only consider beta_0 = 2.5 & beta_2k = 1.0 & beta_3k = 1.0
# This combination is the combination of results reported in the paper
beta_0_v <- 2.5   # -2.5, 0, 2.5 are possible
beta_2k_v <- 1.0  # -1.0, -0.5, 0.5, 1.0 are possible
beta_3k_v <- 1.0  # -1.0, -0.5, 0.5, 1.0 are possible

target_row <- as.numeric(rownames(par.comb[par.comb$beta_0 == beta_0_v & 
                                             par.comb$beta_2k == beta_2k_v & 
                                             par.comb$beta_3k == beta_3k_v, ]))
target_par.comb <- par.comb[target_row, ]
target_results <- results.sim[target_row, drop = FALSE]
rm(results.sim)

##
re_function <- function(results_all = target_results, n.par = 1, xxx = "fit_CLR") {
  ...
  I <- target_par.comb[n.par, 1]
  n_J <- target_par.comb[n.par, 2]
  beta_0 <- target_par.comb[n.par, 3]
  beta_2k <- target_par.comb[n.par, 4]
  beta_3k <- target_par.comb[n.par, 5]
  ...
  datt1 <- data.frame(est = est,
                      ...
                      else_ew = sum(else_which), # Error or Warning in model output
...
##
all_method <- c("fit_CLR", "fit_GLM", "fit_GEE1", "fit_GEE2",
                "fit_glmer1", "fit_glmer2",
                "fit_glmmTMB1", "fit_glmmTMB2")
all_results <- c()
for (iii in 1:nrow(target_par.comb)) {
  all_results_temp <- c()
  for (kkk in 1:length(all_method)) {
    Re_temp <- re_function(results_all = target_results, n.par = iii, xxx = all_method[kkk])
    Re_temp <- Re_temp$datt1
    Re_temp$xxx <- all_method[kkk]
    Re_temp$beta <- c("beta_2k", "beta_3k")
    Re_temp$I <- target_par.comb[iii, 1]
    Re_temp$n_J <- target_par.comb[iii, 2]
...
reprot_f <- function(dat = all_results, B = 1000) {
  ...
  ##
  Method <- stringr::str_split(all_results$xxx, "_", simplify = T)[,2]
...
saveWorkbook(wb, file = "Simulation in Sec 5.xlsx", overwrite = TRUE)
...
################################################################################
## Results for n = 100
temp_name <- paste0("Simulation in Sec 5_b0(", beta_0_v, ")_b2(", beta_2k_v, ")_b3(", beta_3k_v, ")")

all_results_n100 <- all_results[all_results$I == 100, ]
plot_modality_n100 <- data.frame(
  Method = rep(stringr::str_split(all_results_n100$xxx, "_", simplify = T)[,2], 4),
...
plot_modality_n100$Target <- ifelse(plot_modality_n100$Target == 3, "3 Readers",
                                    ifelse(plot_modality_n100$Target == 5, 
                                           "5 Readers", "10 Readers"))
...
plot_A <- ggplot(subset(plot_modality_n100_beta2, Metric == "Absolute bias"), 
  ...
  scale_y_continuous(expand = c(0.02, 0), breaks = seq(0, 0.25, 0.05),
                     limits = c(0, 0.25)) +
...
plot_B <- ggplot(subset(plot_modality_n100_beta2, Metric == "Standard error"), 
  ...
  scale_color_manual(values = c("red", "green", "blue"), 
                     labels = c("3 Readers", "5 Readers", "10 Readers")) +
  scale_y_continuous(expand = c(0.02, 0), breaks = seq(0, 0.4, 0.05),
                     limits = c(0, 0.4)) +
  geom_hline(aes(yintercept = 0), linetype = "dashed", color = "grey20", linewidth=0.5) +
  theme_bw() +
...
plot_C <- ggplot(subset(plot_modality_n100_beta2, Metric == "Coverage"), 
  ...
  scale_color_manual(values = c("red", "green", "blue"), 
                     labels = c("3 Readers", "5 Readers", "10 Readers")) +
  scale_y_continuous(expand = c(0.02, 0), breaks = seq(0.55, 1.0, 0.05),
                     limits = c(0.55, 1)) +
...
plot_D <- ggplot(subset(plot_modality_n100_beta2, Metric == "Proportion of failed"), 
  ...
  scale_color_manual(values = c("red", "green", "blue"), 
                     labels = c("3 Readers", "5 Readers", "10 Readers")) +
  scale_y_continuous(expand = c(0.02, 0), breaks = seq(0, 0.18, 0.02),
                     limits = c(0, 0.18)) +
...
## 11 X 8 pdf save: 2X2 PLOT
pdf(paste0(temp_name, "_n100_b2.pdf"), width = 11, height = 8, family = "Times")
(plot_A + plot_B) / 
  (plot_C + plot_D) + 
...
plot_B <- ggplot(subset(plot_modality_n100_beta3, Metric == "Standard error"), 
  ...
  scale_color_manual(values = c("red", "green", "blue"), 
                     labels = c("3 Readers", "5 Readers", "10 Readers")) +
  scale_y_continuous(expand = c(0.02, 0), breaks = seq(0, 0.40, 0.05),
                     limits = c(0, 0.40)) +
  geom_hline(aes(yintercept = 0), linetype = "dashed", color = "grey20", linewidth=0.5) +
  theme_bw() +
  ggtitle("Standard error") + 
...
plot_C <- ggplot(subset(plot_modality_n100_beta3, Metric == "Coverage"), 
  ...
  scale_color_manual(values = c("red", "green", "blue"), 
                     labels = c("3 Readers", "5 Readers", "10 Readers")) +
  scale_y_continuous(expand = c(0.02, 0), breaks = seq(0.50, 1.0, 0.05),
                     limits = c(0.50, 1)) +
  geom_hline(aes(yintercept = 0.95), linetype = "dashed", color = "grey20", linewidth=0.5) +
  theme_bw() +
  ggtitle("Coverage") + 
...
## 11 X 8 pdf save: 2X2 PLOT
pdf(paste0(temp_name, "_n100_b3.pdf"), width = 11, height = 8, family = "Times")
(plot_A + plot_B) / 
  (plot_C + plot_D) + 
  plot_layout(heights = c(1, 1)) & 
  theme(plot.margin = margin(t = 2, r = 3, b = -10, l = 3))
...
plot_modality_n500 <- data.frame(
  Method = rep(stringr::str_split(all_results_n500$xxx, "_", simplify = T)[,2], 4),
...
plot_modality_n500$Target <- ifelse(plot_modality_n500$Target == 3, "3 Readers",
                                    ifelse(plot_modality_n500$Target == 5, 
                                           "5 Readers", "10 Readers"))
...
plot_B <- ggplot(subset(plot_modality_n500_beta2, Metric == "Standard error"), 
  ...
  scale_color_manual(values = c("red", "green", "blue"), 
                     labels = c("3 Readers", "5 Readers", "10 Readers")) +
  scale_y_continuous(expand = c(0.02, 0), breaks = seq(0, 0.16, 0.04),
                     limits = c(0, 0.16)) +
  geom_hline(aes(yintercept = 0), linetype = "dashed", color = "grey20", linewidth=0.5) +
  theme_bw() +
  ggtitle("Standard error") + 
...
plot_C <- ggplot(subset(plot_modality_n500_beta2, Metric == "Coverage"), 
  ...
  scale_color_manual(values = c("red", "green", "blue"), 
                     labels = c("3 Readers", "5 Readers", "10 Readers")) +
  scale_y_continuous(expand = c(0.02, 0), breaks = seq(0.05, 1.0, 0.05),
                     limits = c(0.05, 1)) +
  geom_hline(aes(yintercept = 0.95), linetype = "dashed", color = "grey20", linewidth=0.5) +
  theme_bw() +
  ggtitle("Coverage") + 
...
## 11 X 8 pdf save: 2X2 PLOT
pdf(paste0(temp_name, "_n500_b2.pdf"), width = 11, height = 8, family = "Times")
(plot_A + plot_B) / 
  (plot_C + plot_D) + 
  plot_layout(heights = c(1, 1)) & 
  theme(plot.margin = margin(t = 2, r = 3, b = -10, l = 3))
...
plot_A <- ggplot(subset(plot_modality_n500_beta3, Metric == "Absolute bias"), 
  ...
  scale_color_manual(values = c("red", "green", "blue"), 
                     labels = c("3 Readers", "5 Readers", "10 Readers")) +
  scale_y_continuous(expand = c(0.02, 0), breaks = seq(0, 0.24, 0.04),
                     limits = c(0, 0.24)) +
  geom_hline(aes(yintercept = 0), linetype = "dashed", color = "grey20", linewidth=0.5) +
  theme_bw() +
  ggtitle("Absolute bias") + 
...
## 11 X 8 pdf save: 2X2 PLOT
pdf(paste0(temp_name, "_n500_b3.pdf"), width = 11, height = 8, family = "Times")
(plot_A + plot_B) / 
  (plot_C + plot_D) + 
  plot_layout(heights = c(1, 1)) & 
  theme(plot.margin = margin(t = 2, r = 3, b = -10, l = 3))
